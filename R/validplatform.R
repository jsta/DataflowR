...
#   params<-"chlext"#
  ...
  if(all(is.na(streamdt$datetime))){
    streamdt$datetime<-jsta::date456posix(streamdt$date,century="20")
  }
  ...
  if(length(dbfile)==0){
    ...
    date_min <- as.character(jsta::date456posix(streamget(yearmon)[1,"date"], century = 20) - (60 * 60 * 24 * tolerance))
    date_max <- as.character(jsta::date456posix(streamget(yearmon)[1,"date"], century = 20) + (60 * 60 * 24 * tolerance))
