...
surfget <- function(rnge, param, fdir = getOption("fdir")){
  ...
  flist <- suppressWarnings(flist[which(as.numeric(substring(basename(flist),1,6)) >= rnge[1])])
  flist <- suppressWarnings(flist[which(as.numeric(substring(basename(flist),1,6)) <= rnge[2])])
